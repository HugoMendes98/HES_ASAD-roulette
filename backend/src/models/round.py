...
class Round(db.Model):
	...
	state = db.Column(
		db.Integer, nullable=False, default=RoundStates.IDLE.value
	)
	...
	def get_bet_on_inOutBet(self, player_bet: InOutBets, player):
		player_bid = Bid.get_bids_from_round_with_bet(
			round=self, player_bet=player_bet
		)
		if player_bid:
	...
	def update_winning_slot(
		self, new_winning_slot: Slots, is_txn=False
	) -> bool:
		self.canceled_check()
	...
	def update_state(
		self, new_state, next_state_timestamp=None, is_txn=False
	) -> bool:
		self.canceled_check()
	...
	def get_history(self, limit=20):
		rounds = (
			...
			.filter(Round.winning_slot.isnot(None))
			.filter(Round.id.isnot(self.id))
			.order_by(Round.round_number.desc())
			.limit(limit)
		)

		return [round.winning_slot for round in rounds]
	...
	def to_dict(self):
		...
		round_dict = {
			...
			"last_win": self.get_history(20),
		}

		if (
			state == RoundStates.BIDABLE
			or state == RoundStates.WAITING
			or state == RoundStates.RESULT
		):
			bets = [
				{
					"userId": bet.user.id,
