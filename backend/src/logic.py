...
def event_loop(app):
	with app.app_context():
		...
		current_game.go_to_idle(
			datetime.utcnow() + timedelta(seconds=IDLE_time_s)
		)
		...
		while True:
			...
			if RoundStates(current_round.state) == RoundStates.IDLE:
				if (
					current_round.next_state_timestamp - datetime.utcnow()
				).total_seconds() < 0:
					print("Les paris sont ouverts")
					winning_slot = None
					pay_out = None
					current_game.go_to_bidable(
						datetime.utcnow() + timedelta(seconds=BIDABLE_time_s)
					)
				...
				if (
					...
					current_game.go_to_waiting(
						datetime.utcnow() + timedelta(seconds=WAITING_time_s)
					)
				...
				if (
					...
					print(
						"Les résultats sont tombés. winning_slot",
						winning_slot,
						"results:",
						pay_out,
					)
